...
function Navbar() {
    ...
    const linkList = ['Services', 'AboutUs', 'Insights', 'Contact'];

    return (
        <div className='w-full py-8 px-8 lg:20 flex gap-3 flex-col lg:flex-row lg:justify-between lg:items-center font-["Neue Montreal"]'>
            <div className=''>
...
function Links({links=[]}){
    return (
        ...
        <div className='flex'>
            {
                links.slice(0,links.length - 1).map( link => <Link key={link} link={link}/>)
            }
        </div>
        <>
...
function Link({link, gap=false}){

    return (
        <a href="/" className={`text-md  font-light ml-7 text-bold text-wrap `}> {link} </a>
    );
};
